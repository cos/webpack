...
class Compilation extends Tapable {
	...
	processDependenciesBlockForChunk(module, chunk) {
		...
		// Start with the provided module
		const queue = [{
			block,
			module,
			chunk
		...
		chunk = initialChunk;
		let availableModules = new StackedSetMap();
		...
		while(queue2.length) {
			...
			// 1. Get the edges at this point of the graph
			const deps = chunkDependencies.get(chunk);
			if(!deps) continue;
			if(deps.length === 0) continue;

			// 2. Create a new Set of available modules at this points
			newAvailableModules = availableModules.createChild(4);
			for(const m of chunk.modulesIterable)
				newAvailableModules.add(m);

			// 3. Filter edges with available modules
